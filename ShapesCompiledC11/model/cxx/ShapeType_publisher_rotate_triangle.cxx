...
void publisher_main(int domain_id, int sample_count)
{
    ...
    ShapeTypeExtended sample( "ORANGE", 0, 0, 30, ShapeFillKind::SOLID_FILL, 0);
	...
	sample.x(int32_t(sample.x() + 125)); // + offset swim late
	...
	sample.angle(int32_t(sample.angle()+rdelta));
        if (sample.x() > xmax || sample.x() < 0) xdelta = xdelta * -1;
        if (sample.y() > ymax || sample.y() < 0) ydelta = ydelta * -1;

        std::cout << "Writing Triangle ShapeTypeExtended, count " << count << std::endl;
